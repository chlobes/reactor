...
const WATER_TRANSFER_RATE: f32 = WATER_REFILL_RATE * 60.0;
const REACTOR_MASS: f32 = 10.0;
const EVAPORATION_RATE: f32 = WATER_REFILL_RATE * 1.0;
...
impl Reactor {
	...
	pub fn tick(&mut self) {
		...
		self.heat += DT * (NEUTRON_HEAT_RATIO * heat_exchange_rate - HEAT_DISSIPATION_RATE) / (REACTOR_MASS + self.water);
		self.fuel = self.fuel.max(0.0).min(3.0);
		...
		if self.water_tank.unlocked {
			...
			let amount = DT * self.water_tank.valve * WATER_TRANSFER_RATE * (self.water_tank.water / 25.0).powf(WATER_PRESSURE); //from water tank
	...
	pub fn vertices(&self) -> Vec<Vertex> {
		...
		quad(v, vec3(panel_pos.x + bar_gap + bar_spacing * 2.0, -bar_size.y / 2.0, 4.0), vec2(bar_size.x, bar_size.y * self.neutrons / 3.0), Color(WHITE));
	...
	pub fn click(&mut self, m: Vec2<f32>) {
		...
		if rect(m, -panel_size / 2.0, panel_size) {
			...
			if rect(m, self.fuel_valve_pos() + vec2(0.005, 0.005), self.fuel_valve_size() - vec2(0.01, 0.01)) {
...
impl WaterTank {
	...
	fn tick(&mut self) {
		...
		let heat_reduction = 3.0 - self.heat; //water boils at heat 3
	...
	fn render(&self, v: &mut Vec<Vertex>) {
		...
		let panel_size = self.panel_size();
		quad(v, panel_pos.extend(1.0), panel_size, Color(panel_color));
		quad(v, (panel_pos + 0.01).extend(2.0), vec2(panel_size.x - 0.02, (panel_size.y - 0.02) / 25.0 * self.water), Color(BLUE));
	...
	fn valve_pos(&self) -> Vec2<f32> {
		vec2(-0.4, 0.27)
	}
	
	fn valve_size(&self) -> Vec2<f32> {
		vec2(0.1, 0.1)
	}
